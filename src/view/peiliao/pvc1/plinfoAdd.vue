    ...
    <Card>
      <p slot="title" style="height: 25px;">PVC配料记录表（一）
        <Button type="success" size="small" style="margin-left: 20px; float: right;" @click="this.reload">
          <Icon type="md-refresh" />清空数据</Button>
      ...
      <Card>
        <Form ref="data" :model="data" :rules="ruleValidate">
          ...
          <Row class="top">
            ...
            <FormItem prop="stirStartTime">
              <DatePicker :value="data.stirStartTime" type="datetime" format="yyyy-MM-dd HH:mm" :editable='false'
                @on-change="data.stirStartTime=$event" placeholder="点击选择时间" style="width: 100%"></DatePicker>
            </FormItem>
            </Col>
            <Col span="3" offset="1" class="intro">结束搅拌时间:</Col>
            <Col span="4">
            <FormItem prop="stirEndTime">
              <DatePicker :value="data.stirEndTime" type="datetime" format="yyyy-MM-dd HH:mm" :editable='false'
                @on-change="data.stirEndTime=$event" placeholder="点击选择时间" style="width: 100%"></DatePicker>
            </FormItem>
            </Col>
          ...
          <Row class="top">
            ...
            <Button type="primary" size="small" style=" float: right;" @click="addRow()">
              <Icon type="md-add" />新增行</Button>
            </Col>
          </Row>
          <Row class="top">
            <Col span="24">
            ...
            <table style="width:100%;text-align: center;" border="1" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td v-for='item in data.detTime'>
                  <TimePicker :value="item.dettime" format="HH:mm" @on-change="item.dettime=$event" placeholder="检测时间"
        ...
        <Card>
          ...
          <Row>
            ...
            <Col span="5">
            <DatePicker type="datetime" format="yyyy-MM-dd HH:mm" :value="data.restStartTime" @on-change="data.restStartTime=$event"
              placeholder="点击选择时间" style="width: 100%"></DatePicker>
            </Col>
          ...
          <Row class="top">
            ...
            <Col span="5">
            <DatePicker type="datetime" format="yyyy-MM-dd HH:mm" :value="data.restEndTime" @on-change="data.restEndTime=$event"
              placeholder="点击选择时间" style="width: 100%"></DatePicker>
            </Col>
            <Col span="2" offset="1" class="intro">班次:</Col>
            <Col span="5">
    ...
    methods: {
      ...
      changeTime(time) {
        ...
        for (var i = 0; i < this.data.detTime.length - 1; i++) {
          all = all + 30;
          var newhour = (Math.floor(all / 60)).toString().length < 2 ? "0" + (Math.floor(all / 60)).toString() : (Math.floor(
            all / 60)).toString();
      ...
      getNumber() {
        if (this.data.formDate != '' && this.data.shift != '' && this.data.factory != '') {
          ...
          if (this.data.restTankNumber != '') {
            ...
            if (a > 1 && a < 10) {
              this.data.lotNumber = this.data.recipe + aa + attDate[1] + attDate[2] + this.data.shift + '0' + this.data
                .restTankNumber;
            } else {
