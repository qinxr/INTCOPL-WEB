		...
		<Card>
			...
			<Form ref="data" :model="data" :rules="ruleValidate">
				...
				<Row class="top">
					...
					<Col span="5">
					<Input v-model="data.totalCount" disabled="" style="width: 100%" />
					</Col>
			...
			<Card>
				...
				<Row class="top">
					...
					<Col span="2" offset="1" class="intro">缓冲罐号:</Col>
					<Col span="3">
					<Input v-model="data.bufferNumber" style="width: 100%" />
					</Col>
					<Col span="2" offset="1" class="intro">二次过滤罐号:</Col>
					<Col span="3">
					<Input v-model="data.secFilNumber" style="width: 100%" />
					</Col>
				...
				<Row>
					...
					<Col span="2" offset="2" class="intro">一层:</Col>
					<Col span="3">
					<Select v-model="data.secFiltration1" style="width:100%">
						<i-option v-for="item in filtration" :value="item.value" :key="item.value">{{ item.label }}</i-option>
					</Select>
					</Col>
					<Col span="2" offset="2" class="intro">二层:</Col>
					<Col span="3">
					<Select v-model="data.secFiltration2" style="width:100%">
						<i-option v-for="item in filtration" :value="item.value" :key="item.value">{{ item.label }}</i-option>
					</Select>
					</Col>
					<Col span="2" offset="2" class="intro">三层:</Col>
					<Col span="3">
					<Select v-model="data.secFiltration3" style="width:100%">
						<i-option v-for="item in filtration" :value="item.value" :key="item.value">{{ item.label }}</i-option>
					</Select>
					</Col>
				</Row>
				<Row class="top">
					<Col span="2" class="intro">过滤开始时间:</Col>
					<Col span="3">
					<DatePicker :value="data.secFilStartTime" type="datetime" format="yyyy-MM-dd HH:mm" @on-change="data.secFilStartTime=$event" placeholder="Select time" style="width: 100%"></DatePicker>
					</Col>
					<Col span="2" offset="1" class="intro">过滤结束时间:</Col>
					<Col span="3">
					<DatePicker :value="data.secFilEndTime" type="datetime" format="yyyy-MM-dd HH:mm" @on-change="data.secFilEndTime=$event" placeholder="Select time" style="width: 100%"></DatePicker>
					</Col>
          ...
          <Select v-model="data.outputReviewer" style="width:100%">
          	<i-option v-for="item in operator" :value="item.value" :key="item.value">{{ item.label }}</i-option>
				...
				<!-- <Row class="top">
					...
					<Col span="2" offset="4" class="intro">带班长:</Col>
					<Col span="4">
					<Select v-model="data.outputReviewer" style="width:100%">
						<i-option v-for="item in inspectors" :value="item.value" :key="item.value">{{ item.label }}</i-option>
					</Select>
	...
	export default {
		...
		watch: {
			...
			'data.shift': {
				handler(newVal, oldVal) {
					...
					switch(newVal) {
						case 'A':
							this.data.cpsReviewer = '马文桂';
							this.data.filInspector = '马文桂';
							this.data.vacReviewer = '马文桂';
							this.data.outputReviewer = '马文桂';
							var a = ['马文桂','刘长涛', '马超', '聂培珍'];
							this.changePeople(this.operator, a);
							this.changePeople(this.inspectors, a);
							break;
						case 'B':
							this.data.cpsReviewer = '马华亭';
		...
		methods: {
			addpl() {
				let index = parseInt(this.$route.params.id.toString());
				var that = this;
				axios.post(that.pub.url + '/pladminSec/editpl/' + index, that.data)
					.then(function(response) {
